...
case class QueryMeta(
    override val rangeStart: Long = 1,
  ...
  def toHeaders: Seq[(String, String)] = {
    var response = totalCount match {
      case None =>
        Seq(
          "Content-Range" -> s"entities $rangeStart-${rangeEnd.getOrElse(0)}")
      case Some(x: Long) =>
        Seq(
...
object QueryMeta {
  ...
  def fromParams(params: Map[String, Seq[String]],
    ...
    val sort: Option[String] =
      params.get("sortField").orElse(params.get("sort")).map(_.head)
    ...
    val qWithRange =
      if (params.contains("rangeStart") && params.contains("rangeEnd")) {
        ...
        val q: QueryMeta = QueryMeta(rangeStart,
                                     Some(rangeEnd),
                                     sort,
                                     sortAsc,
                                     filename = filename)
