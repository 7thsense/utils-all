  ...
  class GenericMapMaker[T] extends MapMaker[T] {
    def make[V](db: DBWrapper, name: String): java.util.AbstractMap[T, V] = {
      db.handle
        ...
        .counterEnable()
        .make[T, V]()
        .asInstanceOf[java.util.AbstractMap[T, V]]
    }
  }

  trait MapMakerLowPriority {
    implicit def getGenericMapMaker[T]: GenericMapMaker[T] =
      new GenericMapMaker[T]
  }

  object MapMaker extends MapMakerLowPriority {
    implicit object StringMapMaker extends MapMaker[String] {
      def make[V](db: DBWrapper,
                  name: String): java.util.AbstractMap[String, V] = {
        db.handle
          .createTreeMap(name)
          .counterEnable()
          .makeStringMap[V]()
          .asInstanceOf[java.util.AbstractMap[String, V]]
      }
    }

    implicit object LongMapMaker extends MapMaker[Long] {
      def make[V](db: DBWrapper,
                  name: String): java.util.AbstractMap[Long, V] = {
        db.handle
          .createTreeMap(name)
          .counterEnable()
          .makeLongMap[V]()
          .asInstanceOf[java.util.AbstractMap[Long, V]]
      }
    }

    implicit object DoubleMapMaker extends MapMaker[Double] {
      def make[V](db: DBWrapper,
                  name: String): java.util.AbstractMap[Double, V] = {
        db.handle
          .createTreeMap(name)
          .comparator(Ordering.Double)
          .counterEnable()
          .make[Double, V]()
          .asInstanceOf[java.util.AbstractMap[Double, V]]
      }
    }
  }
}
...
)(implicit mapMaker: MapDBOffHeapMap.MapMaker[K])
    extends OffHeapMap[K, V] {

  val db = dbParam.getOrElse(MapDBHelper.db)

  private val underlying: util.AbstractMap[K, V] =
    mapMaker.make(db, this.hashCode().toString)

  override def entryIterator: Iterator[(K, V)] =
